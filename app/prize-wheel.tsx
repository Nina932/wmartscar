...
export default function PrizeWheel({rotation,spinning,disabled,demo,kind='meal',completed=false,stepLabel,floatTitle,floatPrize,onSpin}:Props){
 ...
 function openFocus(e:React.PointerEvent<HTMLDivElement>){if(e.pointerType==='mouse'||e.pointerType==='touch'||e.pointerType==='pen')setFocused(true)}
 return <div className={'premium-wheel-stage '+(spinning?'running ':'')+(celebrate?'celebrating ':'')+(focused?'is-focused ':'')} onPointerEnter={openFocus} onPointerDown={openFocus} onPointerMove={move} onPointerLeave={()=>setTilt({x:0,y:0})}>
 {focused&&<><button className="wheel-focus-backdrop" aria-label="ბორბლის ფოკუსის დახურვა" onClick={()=>setFocused(false)}/><button className="wheel-focus-close" aria-label="დახურვა" onClick={()=>setFocused(false)}><X size={22}/></button></>}
